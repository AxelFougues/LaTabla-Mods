...
#endif
static const string nameOfWorld = "LorenzWorld";
using namespace std;
 ...
 */
class LorenzWorld : public GameWorld{
public:
    
    LorenzSystem lSystem = LorenzSystem();
    vec2 centerPoint = vec2(50,30);
    enum States{
        construct, wait, timed
    };
    States state = construct;
    float sideBarWidth = 20;
    int timer = 1000;
    vec2 verticalToken, horizontalToken;
    
    //Universal
    void initialize();
    LorenzWorld(){initializeTime();
        initialize();}
