...
vec2 LorenzWorld::resolveCollisionWithContours ( vec2 point, float radius, const Shape* s ){
...
vec2 LorenzWorld::resolveCollisionWithInverseContours( vec2 point, float radius, const Shape* s ){
    const Contour* in = mContours.findLeafContourContainingPoint(point,mContourFilter) ;
    // ok, find closest
    if (in){
        if ( !in->mIsHole ){
...
vec2 LorenzWorld::getDenoisedShapeVel( const Shape& s) const{
    vec2 vel;
...
void LorenzWorld::updatePhysics(){
...
vec2 LorenzWorld::unlapEdge( vec2 p, float r, const Contour& poly, const Shape* s ){
    float dist ;
...
vec2 LorenzWorld::unlapHoles( vec2 p, float r, ContourKind kind, const Shape* s ){
    float dist ;
    vec2 x ;
...
void LorenzWorld::onShapeShapeCollide( const Shape& a, const Shape& b ){
    ShapeShapeCollisions.push_back( ShapeShapeCollision(getShapeIndex(&a),getShapeIndex(&b)));
}

void LorenzWorld::onShapeContourCollide( const Shape& a, const Contour& b, vec2 pt ){
    ShapeContourCollisions.push_back(ShapeContourCollision( getShapeIndex(&a), mContours.getIndex(b), pt ));
}

void LorenzWorld::onShapeWorldBoundaryCollide  ( const Shape& a, vec2 pt ){
    ShapeWorldCollisions.push_back( ShapeWorldCollision(getShapeIndex(&a),pt) );
}

void LorenzWorld::scaleShapeVelsForIntegrationSteps( int oldSteps, int newSteps ){
...
void LorenzWorld::resolveShapeContourCollisions(){
    for( auto &b : shapes ){
...
void LorenzWorld::resolveShapeCollisions(){
    if ( shapes.size()==0 )
        return ;
