#include "../LorenzWorld.h"
    ...
    float LorenzWorld::angleBetweenPoints(vec2 p0, vec2 p1, vec2 p2 ){
    ...
    float LorenzWorld::distance(vec2 p1, vec2 p2){
        return abs(p1.x-p2.x)+abs(p1.y-p2.y);
    }

    bool LorenzWorld::angleIsValey(vec2 p1, vec2 p2, vec2 p3, vec2 center){
    ...
    bool LorenzWorld::angleIsPeak(vec2 p1, vec2 p2, vec2 p3, vec2 center){
        float dc = distance(p2, center);
        float angle = angleBetweenPoints(p1, p2, p3);
        return (distance(p1, center)< dc && distance(p3, center)< dc) && angle > PEAK_ANGLE_RANGE.x && angle < PEAK_ANGLE_RANGE.y;
    }

    int LorenzWorld::detectConsecutiveFingers(int roundness, Contour c){
    ...
    int LorenzWorld::totalFingers(int roundness, Contour c){
        return abs(detectConsecutiveFingers(roundness, c));
    }

    void LorenzWorld::allGestureRecon(){
        wGestureRecon();
        oGestureRecon();
        vGestureRecon();
    }

    void LorenzWorld::wGestureRecon(){
        wDetected = false;
        wGestures.clear();
    ...
    void LorenzWorld::vGestureRecon(){
        vDetected = false;
        vGestures.clear();
        for(Contour c : mContours){
    ...
    void LorenzWorld::oGestureRecon(){
        oDetected = false;
        oGestures.clear();
        for(Contour c : mContours){
            if(c.mIsHole && c.mPolyLine.size()<CONTOUR_THRESHOLD){
    ...
    void LorenzWorld::drawAllGestures(Color color, bool details){
        drawOGestures(color, details);
        drawVGestures(color, details);
        drawWGestures(color, details);
    }

    void LorenzWorld::drawOGestures(Color color, bool details){
        //if(oDetected){
            gl::color(color);
    ...
    void LorenzWorld::drawVGestures(Color color, bool details){
        //if(vDetected){
            gl::color(color);
    ...
    void LorenzWorld::drawWGestures(Color color, bool details){
        //if(wDetected){
            gl::color(color);
